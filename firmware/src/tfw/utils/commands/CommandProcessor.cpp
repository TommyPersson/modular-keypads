...
ParsedCommand CommandProcessor::parseCommand(const std::string_view& rawCommand) {
    tfw::utils::allocations::ArenaAllocator<std::string_view> allocator(this->arena);

    const auto commandParts = tfw::utils::allocations::arena::strings::split(rawCommand, ':', allocator);

    const auto commandId = strings::atol(commandParts[0]);
    const auto command = commandParts[1];
    auto args = commandParts.size() > 2
                    ? tfw::utils::allocations::arena::strings::split(commandParts[2], ',', allocator)
                    : tfw::utils::allocations::arena::vector(allocator);
